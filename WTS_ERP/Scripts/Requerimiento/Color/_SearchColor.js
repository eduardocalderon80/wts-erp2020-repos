...
var app_searchColor =
    (
        function (d, idpadre) {
            var ovariables = {
                ...
                aRows: [],
                aRowsSelected: [],
                idTable: 'tbl_searchColor',
                isCheckBox: true, // [true: CheckBox, false: Radio]
                numPagina: 5,
                idTxtParametro: 'txtpar_searchColor',
                urlGet: 'Requerimiento/Color/GetColorSearchAll_csv',
            ...
            function DataTableTemplate(arr) {
                const html = arr.map(x => {
                    return `<tr data-par="idColor:${x.IdRequerimientoDetalle},idrequerimiento:${x.IdRequerimientoDetalle}">
                                <td><input class="i-check" type="radio" data-id="${x.IdRequerimientoDetalle}" name="input_search_select" /></td>
                                <td>${x.Nota}</td>
                                <td>${x.ComboName}</td>
            ...
            function load() {
                ...
                // Events
                _('btnsearchColorAdd').addEventListener('click', fn_copiar_Color, false);
                // Parameter
                cargaParameter();
            }

            function fn_copiar_Color() {
                let arr_chk_select = Array.from(_('tbody_tbl_searchColor').getElementsByClassName('cls_chk_select')).filter(x => x.classList.value.indexOf('checked') >= 0);
                if (arr_chk_select.length > 0) {
                    let datapar = arr_chk_select[0].parentNode.parentNode.getAttribute('data-par');
                    let idrequerimientoDetalle = _par(datapar, 'idrequerimiento');
                    $("#modal_SearchColor").modal('hide');                    
                    appStages.fn_newcolor(-idrequerimientoDetalle,0);
                  
                } else {
                    _swal({ mensaje: 'Seleccione el color porfavor...!', estado: 'error' });
                }
            }
            ...
            function req_ini() {
                const parametro = { IdCliente: ovariables.idcliente };// => searchtrim : obtener los parametros
    ...
    )(document, 'panelEncabezado_searchColor');

(
    function ini() {
        app_searchColor.load();
        app_searchColor.req_ini();
    }
)();
