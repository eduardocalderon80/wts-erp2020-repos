var appFacturacionSampleInicial_Pestania_fc = (
    function (d, idpadre) {
        var ovariables = {
            idprograma: '',
        ...
        function page_result(padata, indice) {  // padata es la data parseada databdenbruto.JSON.parse(databrutodesdebd)
            ...
            if (padata != null && padata.length > 0) {
                ...
                if (oUtil.indiceactualbloque > 0) {
                    contenido += "<li data-indice='-1'> <a onclick='appFacturacionSampleInicial_Pestania_fc.paginar(-1, appFacturacionSampleInicial.fn_pintar_tabla_facturacliente_detale_ini);' > << </a></li>";
                    contenido += "<li data-indice='-2'> <a onclick='appFacturacionSampleInicial_Pestania_fc.paginar(-2, appFacturacionSampleInicial.fn_pintar_tabla_facturacliente_detale_ini);' > < </a></li>";
                }
                ...
                for (let i = inicio; i < fin; i++) {
                    if (i <= indiceultimapagina) {
                        ...
                        contenido += `<a class='${clsactivo}' onclick='appFacturacionSampleInicial_Pestania_fc.paginar(`;
                        contenido += i.toString();
                        contenido += `, appFacturacionSampleInicial.fn_pintar_tabla_facturacliente_detale_ini);'>`;
                ...
                if (oUtil.indiceactualbloque < indiceultimobloque) {
                    contenido += "<li data-indice='-3'> <a onclick='appFacturacionSampleInicial_Pestania_fc.paginar(-3, appFacturacionSampleInicial.fn_pintar_tabla_facturacliente_detale_ini);' > > </a></li>";
                    contenido += "<li data-indice='-4'> <a onclick='appFacturacionSampleInicial_Pestania_fc.paginar(-4, appFacturacionSampleInicial.fn_pintar_tabla_facturacliente_detale_ini);' > >> </a></li>";
                }
            }
            ...
            for (let i = 0; i < nfilters; i++) {
                ...
                if (filter.type == "text") {
                    ...
                    filter.onkeyup = function () {
                        ...
                        appFacturacionSampleInicial.fn_pintar_tabla_facturacliente_detale_ini(oUtil.adataresult, oUtil.indiceactualpagina);
                    }
                }
            }
        }
...
(
    function ini() {
        appFacturacionSampleInicial_Pestania_fc.load();
