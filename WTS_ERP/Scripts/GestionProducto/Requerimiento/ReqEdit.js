var app_Requerimiento_Edit = (
     ...
     function (d, idpadre) {
         ...
         function load() {
             ...
             $('#div_fecha_inicial .input-group.date').datepicker('update', moment().format('MM/DD/YYYY'));
             $('#div_fecha_fin .input-group.date').datepicker('update', moment().add(1, 'month').format('MM/DD/YYYY'));
             ...
             $('#cbo_analista').chosen().change(function () {
                 let cbo_analista = $("#cbo_analista").chosen().val() == null ? "0" : $("#cbo_analista").chosen().val();
                 ovariables.arr_id_analista_selection = [];

                 if (cbo_analista != '0') {
                     cbo_analista.forEach(x=> {
                         obj = {},
                         obj.idpersonal = parseInt(x);
                         ovariables.arr_id_analista_selection.push(obj);
                     })
                 }
                 else { ovariables.arr_id_analista_selection = ovariables.arr_id_analista; }
             });
             ...
             _Get(urlaccion)
                 .then((response) => {
                     ...
                     if (rpta !== null) {
                         ...
                         ovariables.arr_analista = rpta[0].analista != '' ? JSON.parse(rpta[0].analista) : [];
                         ovariables.arr_analistacliente = rpta[0].analistacliente != '' ? JSON.parse(rpta[0].analistacliente) : [];
                     }
         ...
         function fn_load_analista() {
             let arr_id_cliente_selection = ovariables.arr_id_cliente_selection,
                 ...
                 arr_analista = ovariables.arr_analista,
                 arr_analistacliente = ovariables.arr_analistacliente,
                 cbo_analista = '';

             let resultado_analistacliente = [],
                 resultado_analista = [];

             resultado_analistacliente = arr_analistacliente.filter(j=> arr_id_cliente_selection.some(t=> { return (t.idcliente === j.idcliente) }));            
             resultado_analista = arr_analista.filter(i=> resultado_analistacliente.some(s=> { return (s.idpersonal === i.idpersonal) }));

             if (resultado_analista.length > 0) {
                 resultado_analista.forEach(x=> {
                     obj = {},
                     obj.idpersonal = x.idpersonal;
                     ovariables.arr_id_analista.push(obj);
                     cbo_analista += `<option value='${x.idpersonal}'>${x.personal}</option>`;
                 });

                 $(".chosen-container").css("pointer-events", "");
                 $(".chosen-choices").css("background-color", "");
             }
             else { ovariables.arr_id_analista = []; }

             ovariables.arr_id_analista_selection = ovariables.arr_id_analista;
             _('cbo_analista').innerHTML = cbo_analista;
             $(".chosen-select").trigger('chosen:updated');
         }
         ...
         function fn_load_estilo() {
             let arr_id_cliente_selection = ovariables.arr_id_cliente_selection,
                ...
                cbo_estilo = `<option value='0'>Seleccione Estilo</option>`;
             ...
             if (resultado_estilo.length > 0) {
                 resultado_estilo.forEach(x=> { cbo_estilo += `<option value='${x.codigoestilo}'>${x.estilo}</option>`; });
             }
         ...
         */
         ...
         function req_info() {
             ...
             let analista = '';
             ovariables.arr_id_analista_selection.forEach(x=> { analista += x.idpersonal + ','; });
             analista = analista.slice(0, -1);
             
             let parametro = { codigoclientetemporada: codigoclientetemporada, idproveedor: idproveedor, idtipomuestra: idtipomuestra, idactividad: idactividad, codigoestado: codigoestado, fechainicio: fechainicio, fechafin: fechafin, chkfecha: chkfecha, cliente: cliente, analista: analista };
         ...
         function req_load_requerimiento() {
             let arr_requerimiento = ovariables.arr_requerimiento,
                   codigoestilo = _('cbo_estilo').value,
             ...
             resultado_requerimiento = arr_requerimiento.filter(j=> (codigoestilo === '0' || j.codigoestilo === codigoestilo));
...
)(document, 'pnl_requerimiento_edit');

(function ini() {
    app_Requerimiento_Edit.load();
    app_Requerimiento_Edit.req_ini();
})();
