var app_Personal_Editar = (
    function (d, idpadre) {
        ...
        function fn_save() {
            ...
            let req_enty = required_item({ id: 'pnl_Personal_Editar', clase: '_enty_data' });
            if (req_enty) {
                swal({
                    html: true,
                    title: "Guardar Datos",
                    ...
                    let err = function (__err) { console.log('err', __err) },
                        parametro = _getParameter({ clase: '_enty_data', id: 'pnl_Personal_Editar' }),
                        frm = new FormData();
        ...
        function fn_crearfoto() {
            if (ovariables.lst_personal[0].imagenwebnombre !== '') {
                let ruta = `${urlBase()}Content/img/RRHH/personal/${ovariables.lst_personal[0].imagenwebnombre}`;
                _('_FotoPerfil').src = ruta
                $("#_FotoPerfil").attr("data-initial", ruta);
            } else {
                let rutax = `${urlBase()}Content/img/RRHH/personal/default.jpg`;
                _('_FotoPerfil').src = rutax
                $("#_FotoPerfil").attr("data-initial", rutax);
            }
        ...
        function fn_load_personal() {
            ...
            if (lst_personal[0].lusuarioasistencia == false) { _('lusuarioasistencia').click(); }
            _('lusuarioasistencia').checked = lst_personal[0].lusuarioasistencia;
...
)(document, 'pnl_Personal_Editar');
(
    function ini() {
        app_Personal_Editar.load();
        app_Personal_Editar.req_ini();
    }
)();
