var appVacacionesNew = (
    function (d, idpadre) {
        var ovariables = {
            ...
            lstferiados: [],
            lstusuariossabados: [],
            totaldiasvacacionesregistradas : 0
        }
        ...
        function load() {
                ...
                e.dates.forEach((x) => {
                    ...
                    $("#taginputspan").append(`<div class="col-xs-12 col-lg-4 col-sm-6 col-md-6 no-padding">
                                                       <div class="input-group">
                                                           ...
                                                           <span class="input-group-addon cursor" onclick='appVacacionesNew.removetag("${fecha}")'>
            ...
            $('#txtFechas').attr("placeholder", "__/__/____");
        ...
        function fn_rangecalendario() {
            $('#rangecalendario .input-daterange input').each(function () {
                $(this)
                    .datepicker({ daysOfWeekDisabled: [0, 6] }).on("changeDate", function (e) {
                        ...
                        if (ovariables.rangofechasdates[0] !== undefined && ovariables.rangofechasdates[0] !== "") {
                            if (ovariables.rangofechasdates[1] !== undefined && ovariables.rangofechasdates[1] !== "") {
                                ...
                                _("txtDias").value = ((ovariables.rangofechasdates[1] - ovariables.rangofechasdates[0]) / (1000 * 3600 * 24)) - cont_domsab + 1;
        ...
        function removetags() {
            $('#txtFechas').datepicker("setDates", null);
            ovariables.seleccionfechas = null;
            ovariables.seleccionfechas = new Array();
            $("#taginputspan").empty();
            $('#hdnFechas').val('');
        ...
        function removetag(fech) {
            ovariables.seleccionfechas = null;
            ovariables.seleccionfechas = new Array();
            let arr = $('#txtFechas').datepicker("getDates");
            ...
            newarr = newarr.filter(function (dato) {
                ovariables.seleccionfechas.push(dato);
                return dato != undefined
            });

            // Se elimina seleccionado
            $('#txtFechas').datepicker("setDates", newarr);
            $('#hdnFechas').val(newarr.join(','));
        ...
        function cuentaFindes(inicio, fin) {
            ...
            let feriados = ovariables.lstferiados;           

            let usuariosabado = (ovariables.lstusuariossabados.filter(x => parseInt(x.idpersonal, 10) === parseInt(window.utilindex.idpersonal, 10)).length > 0) ? 1 : 0;


            for (var i = 0; i < diffDays; i++) {
                //0 => Domingo - 6 => Sábado
                if (ini.getDay() === 0 || (usuariosabado === 0 && ini.getDay() === 6) || (feriados.filter(x => (x.getMonth() === ini.getMonth()) && (x.getDate() === ini.getDate())).length > 0)) {
                    cuentaFinde++;
                }
                            ...
                            <div class="ibox-title">
                                <div class="form-horizontal">
                                    <div id="divBotones" class="text-right">
                                        <button type="button" class="btn btn-info" onclick="appVacacionesNew.${ovariables.modal_callback}()">
        ...
        function fn_return() {
            ...
            if (ovariables.ruta === 'calendario') {
                ...
                let urlAccion = 'RecursosHumanos/Vacaciones/Index';
                _Go_Url(urlAccion, urlAccion);
            }
        }
        ...
        function fn_Check_All_Conditions() {
            ...
            if (mesesDif < ovariables.configuracionvacaciones_plazominimomes) {  //if (ovariables.lstsolicitud.totalmeses < 6) {
                ...
                //} else if (ovariables.totaldiasvacacionesregistradas < parseInt(ovariables.configuracionvacaciones_condiciondiasminimo,10)) {                              
                //    if (cantidad_dias !== 5 && cantidad_dias !== 6) {
                //        if (cantidad_dias < 5) {
                //            swal({ title: "Advertencia", text: "Estas solicitando " + cantidad_dias + " dia/s, debes solicitar periodos de 5, 6 o " + ovariables.configuracionvacaciones_condiciondiasminimo + " a mas días.", type: "warning" });
                //            bool = false;
                //            return bool;
                //        }
                //        else if (cantidad_dias > 6 && cantidad_dias < parseInt(ovariables.configuracionvacaciones_condiciondiasminimo, 10)) {
                //            swal({ title: "Advertencia", text: "Estas solicitando " + cantidad_dias + " dia/s, debes solicitar periodos de 5, 6 o " + ovariables.configuracionvacaciones_condiciondiasminimo + " a mas días.", type: "warning" });
                //            bool = false;
                //            return bool;
                //        }                   
                ...
                let numpalabras = $.grep(_("txtComentario").value.split(' '), function (n) {
                    return (n);
                });

                if (numpalabras.length === 0 || numpalabras.length < 3) {
                    swal({ title: "Advertencia", text: "Estas solicitando " + _("txtDias").value + " dia/s, por lo tanto debes ingresaren en el campo comentario el detalle del motivo.", type: "warning" });
                    bool = false;
                    return bool;
                }
            ...
            } else if (cantidad_dias >= 1 && cantidad_dias < 5) {
                let comentario = _("txtComentario").value;
                let numpalabras = $.grep(_("txtComentario").value.split(' '), function (n) {
                    return (n);
                });
                ...
                if (numpalabras.length === 0 || numpalabras.length < 3) {
                    swal({ title: "Advertencia", text: "Estas solicitando " + _("txtDias").value + " dia/s, por lo tanto debes ingresar en el campo comentario el detalle del motivo.", type: "warning" });
                    bool = false;
                    return bool;
                }

            } else if (cantidad_dias >= 3 && cantidad_dias <= 6) { //ovariables.configuracionvacaciones_plazominimoespecial) {   //} else if (cantidad_dias > 2 && cantidad_dias < 8) {                

                if (cantidad_dias >= 1 && cantidad_dias < 5) {
                    let comentario = _("txtComentario").textContent;
                    let numpalabras = $.grep(_("txtComentario").value.split(' '), function (n) {
                        return (n);
                    });
        ...
        function fn_creartablaadvertenciafechascoincidentes(opcionfechas, fechainicio, fechafin, fechas) {
            ...
            _Post('RecursosHumanos/Vacaciones/GetAllData_FechasCoincidentes_Vacaciones', frm)
                .then((resultado) => {
                    if (resultado !== null) {
                        ...
                        if (lstresultado.length > 0) {
                            ...
                            if (lsttodos !== "") {
                                                ...
                                                <table id='tbl_coincidencias' style='font-size:9pt!important' class='table table-hover table-bordered table-condensed'>
                                                    <thead>
                                                         <tr>
                                                            ...
                                                            <th class='text-center'>Saldo de Vacaciones</th>
                                ...
                                lsttodos.forEach((x) => {
                                    ...
                                    <td class='text-center font-bold'>${x.totalvacaciones}</td>
        ...
        function fn_save() {
            if (fn_Check_All_Conditions()) {
                let req_enty = _required({ clase: '_enty_grabar', id: 'panelEncabezado_VacacionesNew' });
                if (req_enty) {
                    swal({
                        html: true,
                        title: "Guardar Solicitud",
                        text: "¿Estas seguro/a que deseas guardar tu Solicitud de Vacaciones? <br /> <span style='font-weight: 400; font-size: 14px;'>Esta solicitud se enviara de forma automatica a tu Jefe directo para su Aprobación</span>",
                        type: "info",
                        ...
                        _Post('RecursosHumanos/Vacaciones/InsertData_Vacaciones', frm)
                            .then((resultado) => {
                                if (resultado !== null) {
                                    if (resultado === '-1') {
        ...
        function fn_update() {
            ...
            if (req_enty) {
                swal({
                    ...
                    text: "¿Estas seguro/a que deseas actualizar tu solicitud de Vacaciones?",
                    type: "info",
                    showCancelButton: true,
                    confirmButtonColor: "#1c84c6",
                    confirmButtonText: "SI",
                    ...
                    _Post('RecursosHumanos/Vacaciones/InsertData_Vacaciones', frm)
                        .then((resultado) => {
                            if (resultado !== null) {
                                swal({ title: "¡Solicitud Actualizada!", text: "Haz enviado tus vacaciones correctamente", type: "success", timer: 1000, showCancelButton: false, showConfirmButton: false });
                                fn_return();
                            } else {
                                swal({ title: "Ha surgido un problema", text: "Por favor, comunicate con el administrador TIC", type: "error" });
                            }
        ...
        function fn_aprobar_grabar() {
            swal({
                ...
                let parametro = {
                    idsolicitud: ovariables.id,
                    comentario: json.comentario_siaprobar,
                    codigoestado: ovariables.estados.APROBADO
                };
                let frm = new FormData();
                frm.append('par', JSON.stringify(parametro));
                _Post('RecursosHumanos/Vacaciones/UpdateData_Vacaciones', frm)
                    .then((resultado) => {
                        if (resultado !== null) {
                            $('#modal_solicitud').modal('hide');
                            swal({ title: "¡Solicitud Aprobada!", text: "Se actualizo las vacaciones correctamente", type: "success", timer: 1000, showCancelButton: false, showConfirmButton: false });
                            fn_return();
                        } else {
                            swal({ title: "Ha surgido un problema", text: "Por favor, comunicate con el administrador TIC", type: "error" });
                        }
        ...
        function fn_cancelar_grabar() {
            swal({
                ...
                text: "¿Estas seguro/a que deseas Cancelar tus Vacaciones?",
                type: "info",
                showCancelButton: true,
                confirmButtonColor: "#1c84c6",
                confirmButtonText: "SI",
                ...
                _Post('RecursosHumanos/Vacaciones/UpdateData_Vacaciones', frm)
                    .then((resultado) => {
                        if (resultado !== null) {
                            $('#modal_solicitud').modal('hide');
                            swal({ title: "¡Solicitud Cancelada!", text: "Se cancelo tus vacaciones correctamente", type: "success", timer: 1000, showCancelButton: false, showConfirmButton: false });
                            fn_return();
                        } else {
                            swal({ title: "Ha surgido un problema", text: "Por favor, comunicate con el administrador TIC", type: "error" });
                        }
        ...
        function fn_rechazar_grabar() {
            swal({
                ...
                text: "¿Estas seguro/a que deseas Rechazar las vacaciones?",
                type: "info",
                showCancelButton: true,
                confirmButtonColor: "#1c84c6",
                confirmButtonText: "SI",
                ...
                _Post('RecursosHumanos/Vacaciones/UpdateData_Vacaciones', frm)
                    .then((resultado) => {
                        if (resultado !== null) {
                            $('#modal_solicitud').modal('hide');
                            swal({ title: "¡Solicitud Rechazada!", text: "Se rechazaron las vacaciones correctamente", type: "success", timer: 1000, showCancelButton: false, showConfirmButton: false });
                            fn_return();
                        } else {
                            swal({ title: "Ha surgido un problema", text: "Por favor, comunicate con el administrador TIC", type: "error" });
                        }
        ...
        function req_ini() {
            ...
            _Get('RecursosHumanos/Vacaciones/GetData_Inicial?par=' + JSON.stringify(parametro))
                .then((resultado) => {
                    ...
                    if (rpta !== null) {
                        ...
                        ovariables.totaldiasvacacionesregistradas = parseInt(ovariables.totaldiasvacacionesregistradas,10);
                        ...
                        let botones = (_json) => {
                            ...
                            if (_json.length > 0) {
                                _json.forEach(x => {
                                    html += `<button type="button" class="${x.Clase}" onclick="appVacacionesNew.${x.Funcion}()" title="${x.Titulo}" style="margin-right: 5px;">
                        ...
                        if (ovariables.accion !== "edit") {
                            ...
                            if (ovariables.lstsolicitud.fechas !== "") {
                                    ...
                                    arrFechas.forEach(x => {
                                        ...
                                        items += `<div class="col-xs-12 col-lg-4 col-sm-6 col-md-6 no-padding">
                                                       <div class="input-group">
                                                           ...
                                                           <span class="input-group-addon cursor" onclick='appVacacionesNew.removetag("${x}")'>
                                                               <span class="fa fa-trash"></span>
                                                           </span>
                                                       </div>
                                                   </div>`
                                ...
                                _("txtDias").value = ((datefin - dateini) / (1000 * 3600 * 24)) - cont_domsab + 1;
                            }
                        ...
                        Disabled_DatesInDatePicker(ovariables.lstferiados, ovariables.lstusuariossabados);
                    }
                }, (p) => { err(p); });
        }

        function Disabled_DatesInDatePicker(lstferiados, lstusuariossabados) {
            $("#txtFechas").datepicker("setDatesDisabled", lstferiados);
            $("#txtFechaInicio").datepicker("setDatesDisabled", lstferiados);
            $("#txtFechaFin").datepicker("setDatesDisabled", lstferiados);
            if (lstusuariossabados.length > 0) {
                if (lstusuariossabados.filter(x => (parseInt(x.idpersonal, 10) === parseInt(window.utilindex.idpersonal, 10)) && x.trabajasabado === 1).length > 0) {
                    $("#txtFechas").datepicker("setDaysOfWeekDisabled", [0]);
                    $("#txtFechaInicio").datepicker("setDaysOfWeekDisabled", [0]);
                    $("#txtFechaFin").datepicker("setDaysOfWeekDisabled", [0]);
                }
            }
        }
        ...
        function fn_cargartablafechas(_json) {
            ...
            if (data.fechas === "") {
                ...
                let cont_domsab = cuentaFindes(dateini, datefin);
                let dias = ((datefin - dateini) / (1000 * 3600 * 24)) - cont_domsab + 1;
                htmlbody = `<tr>                                    
        ...
        return {
            ...
            fn_Check_All_Conditions: fn_Check_All_Conditions
        }
    }
)(document, 'panelEncabezado_VacacionesNew');
(
    function ini() {
        appVacacionesNew.load();
        appVacacionesNew.req_ini();
    }
)();