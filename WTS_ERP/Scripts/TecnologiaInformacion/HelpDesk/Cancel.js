...
function fn_load_update() {
    let html = `
          <div id='formCancelar' class ='ibox white-bg'>
                ...
                <div class ='ibox-content'>
                    <div class ='form-horizontal'>
                        <div class ='form-group'>
                            <div class ='col-sm-12'>
                                <label class ='control-label'>Coloque un motivo por el cual va a cancelar esta solicitud: </label>
                            </div>
                         </div>
                    </div>
                    <div class ='form-horizontal'>
                        <div class ='form-group'>
                            <div class ='col-sm-12'>
                                <textarea id='txtComentario' class ='form-control col-sm-12 _enty' name="Descripcion" rows="6" style="resize:none" placeholder="Ingresar motivo de cancelación" data-id="Comentario" data-min="1" data-max="200" maxlength="200" data-required='true'></textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        `;
    _('formCancel').innerHTML = html;
}

function req_request() {

    let req = _required({ id: 'formCancelar', clase: '_enty' });
    if (req) {
        swal({
        ...
        swal({ title: "Alert", text: "Debe Ingresar un motivo de cancelación", type: "warning" });
    }
}
...
function req_cancel() {
    ...
    let par = JSON.stringify({ IdSolicitud: parseInt(idsolicitud), IdPersonal: idpersonal, IdAccion: 11 });
...
function res_cancel(response) {
    ...
    if (orpta != null) {
        if (orpta.estado === 'success') {
            swal({ title: "Buen Trabajo!", text: "Usted ha cancelado esta solicitud correctamente", type: "success" });
            fn_close_modal();
            fn_return();
        };
