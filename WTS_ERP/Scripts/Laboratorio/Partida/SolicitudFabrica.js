...
function LoadSolicitudPartida(data) {
    ...
    if (data !== "") {
        ...
        if (Data.SolicitudPartida !== "") {
            ...
            $("#txtCodigoColor").val(SolicitudPartida.CodigoColorTintoreria);// :change
...
//        var html = "";
//        html += "<table id='tblPO' class='table table-bordered'><thead><tr><th>Select</th><th>CodigoPo</th><th>CodigoEstilo</th><th>Lote</th><th>Tela</th><th>Descripcion Tela</th><th>Weight</th><th>Color</th></tr></thead>";
...
function Save() {
    ...
    if (Reproceso == 'S') {       
        if (nPartidaRelacion == 0) { Mensaje += "<tr><td>Si la solicitud de partida es un reproceso debe ingresar una partida.</td></tr>"; }
    }
    ...
    // Luis
    //PoEstilo.filter(x => x.EstadoRegistro === "new").forEach(x => {
    //    PoEstiloArr.push(x);
    //})
    //PoEstilo.filter(x => x.Eliminado === 1).forEach(x => {
    //    PoEstiloArr.push(x);
    //})

    PoEstilo.filter(x => x.Eliminado === 0).forEach(x => { PoEstiloArr.push(x); });
    ...
    if (PoEstiloArr.length > 0) { frm.append("parDetail", JSON.stringify(PoEstiloArr)); }
    else { frm.append("parDetail", ""); }


    if (EstiloArr.length > 0) { frm.append("parSubDetail", JSON.stringify(EstiloArr)); }
    else { frm.append("parSubDetail", ""); }

    if (PartidaRelacionadaArr.length > 0) { frm.append("parFoot", JSON.stringify(PartidaRelacionadaArr)); }
    else { frm.append("parFoot", ""); }
    ...
    Post('Laboratorio/Partida/SolicitudFabricaSave', frm, Alerta);
}
...
function ShowModalPartida() {
    ...
    var par = $("#hfCodigoPartida").val() + "," + $("#txtPartida").val() + "," + _('cboFabrica').value + "," + _('cboCliente').value;
...
function LoadPo(data) {
    if (data !== "[]") {
        ...
        //html += "<table id='tblPO' class='stripe row-border order-column'><thead><tr><th>Select</th><th>CodigoPo</th><th>CodigoEstilo</th><th>Lote</th><th>Tela</th><th>Descripcion Tela</th><th>Weight</th><th>Color</th></tr></thead>";
        for (var i = 0; i < nLotes; i++) {
            html += "<tr>";
...
function handler_check() {
    $('.i-checks').iCheck({
        ...
        if (chk) {
            oVariables.arrInstruccionCuidadoChk.push(obj);
        }
        else {
        ...
        _('txt_InstruccionesCuidadoFabrica').innerHTML = text.slice(0, -3);;
    });
}
...
function fn_CambiarPrueba() {
    //let codigoprueba = _('cboPrueba').value;
    //if (codigoprueba == 'p' || codigoprueba == 'pp' || codigoprueba == 'f' || codigoprueba == 'gf' || codigoprueba == 'gp') {
    //    _('txt_InstruccionesCuidado').disabled = true;
    //    _('txt_InstruccionesCuidado').value = '';
    //}
    //else { _('txt_InstruccionesCuidado').disabled = false; }
}
