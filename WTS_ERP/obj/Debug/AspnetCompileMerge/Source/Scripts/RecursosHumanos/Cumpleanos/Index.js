    ...
    function (d, idpadre) {
        ...
        function req_ini() {
            let err = function (__err) { console.log('err', __err) },
                parametro = { x: 1 };
            _Get('RecursosHumanos/Cumpleanos/GetAllData_Cumpleanos?par=' + JSON.stringify(parametro))
                .then((resultado) => {
        ...
        function fn_cargartodo(tipo, vari, lstsolfilter) {
            ...
            // Limpia pie
            _('ibox-content-ibox-graficopie').innerHTML = '<canvas id="doughnutChart" height="140"></canvas>';
                ...
                data.forEach(x => {
                    htmlbody += `<tr>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <button class="btn btn-sm btn-outline btn-primary" onclick="appCumpleanosIndex.fn_edit(${x.idsolicitud})">
        ...
        function fn_crearpiechart(_json, esjefe, lstsolicfilter) {
            ...
            if (esjefe === 1) {
                ...
                _("h5-ibox-graficopie").classList.add("text-center");
            }
        ...
        function fn_crearbarchart(_json) {
            ...
            for (let i = 0; i < nromeses.length; i++) {
                let cantidad = ovariables.lstsolicitudes.filter(x => (_strToDate(x.fechasolicitud + ' 0:00').getMonth() + 1) === nromeses[i]).length;
                datos.push(cantidad);
            }
...
<th x:autofilter="all" style="background-color: #000000; color: white;">Id Solicitud</th>
            ...
            _createExcel({
                worksheet: 'Reporte Cumpleaños',
                style: '',
                table: html,
                filename: 'Reporte Cumpleaños'
            });
        }
        ...
        function fn_new() {
            let urlAccion = 'RecursosHumanos/Cumpleanos/New';
...
)(document, 'panelEncabezado_CumpleanosIndex');
(
    function ini() {
        appCumpleanosIndex.load();
        appCumpleanosIndex.req_ini();
    }
)();