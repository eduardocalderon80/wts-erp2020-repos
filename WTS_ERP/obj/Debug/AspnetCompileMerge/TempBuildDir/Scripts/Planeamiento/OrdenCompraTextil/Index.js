...
function load() {
    ...
    $('#cboproveedor').on('change', fn_load_proveedor_text);

    _('btnexportexcel').addEventListener('click', req_export_excel)
    btnexportexcel
}
...
function req_export_excel() {
    let codigofabrica = _('cbofabrica').value;
    let codigocliente = _('cbocliente').value;
    let fechadesde = _convertDate_ANSI(_('txtfechadesde').value);
    let fechahasta =  _convertDate_ANSI(_('txtfechahasta').value);
    let idoc = _('cbo_oc').value;

    var par = `codigofabrica:${codigofabrica},codigocliente:${codigocliente},fechadesde:${fechadesde},fechahasta:${fechahasta},idoc:${idoc}`;
    
    let urlaccion = urlBase() + "Planeamiento//OrdenCompraTextil/OrdenCompraTextil_Export_Excel?par=" + par;
    $.ajax({
        type: 'GET',
        url: urlaccion,
        dataType: 'json',
        ...
        if (data.Success) {
            urlaccion = urlBase() + 'Planeamiento//OrdenCompraTextil/descargarexcel_reporte';
...
/* Index */
function req_ini() {
    let urlaccion = `Planeamiento/OrdenCompraTextil/OrdenCompraTextil_Master`;
...
function res_ini(response) {
    ...
    if (orpta != '') {
        ...
        if (orpta[0].proveedor != '') { ovariables.arrproveedor = orpta[0].proveedor; }
        fn_load_fabrica();
        fn_load_cliente();
        req_data();
    }
...
function fn_load_proveedor() {
    let arrproveedor = JSON.parse(ovariables.arrproveedor);
    let cboproveedorhtml = `<option value=''>All Supplier</option>`;
    if (arrproveedor.length > 0) {
        arrproveedor.forEach(x=> { cboproveedorhtml += `<option value='${x.idproveedor}'>${x.nombreproveedor}</option>`; });
    }
    _('cboproveedor').innerHTML = cboproveedorhtml;
    $('#cboproveedor').select2();
}

function fn_load_proveedor_text() {
    let idproveedor = $('#cboproveedor').val();
    let arrproveedor = JSON.parse(ovariables.arrproveedor);
    let resultado = arrproveedor.filter(x=> x.idproveedor.toString() === idproveedor);
    if (resultado.length > 0) { _('txtproveedor').value = resultado[0].nombreproveedor; }
    else { _('txtproveedor').value = '' }
}

function req_data() {
    let par = _getParameter({ id: 'formbusqueda', clase: '_enty' });
    let urlaccion = 'Planeamiento/OrdenCompraTextil/OrdenCompraTextil_List?par=' + JSON.stringify(par);
    ...
    resultadopo.forEach(x=> {
        ...
        htmlbody += `
                <tr data-par='po:${x.po},estilo:${x.estilo},lote:${x.lote},color:${x.color},cantidad:${x.cantidad},codfabrica:${x.codfabrica},codcliente:${x.codcliente}' onclick='event_rows(event)'>
                    ...
                    <td class ='text-center'>${x.dias_sinoct}</td>
...
function fn_send(_arr) {
    ...
    if (tipo == '1') {
        ...
        _arr.forEach(x=> {
            _('txtoc').value = x[0].oc;
            $('#cboproveedor').select2("val", x[0].idproveedor);
            _('txtproveedor').value = x[0].proveedortela;
            _('txtfechaoc').value = x[0].fechaoc;
...
function req_insert() {
    ...
    if (req) {
        swal({
            ...
            let urlaccion = 'Planeamiento/OrdenCompraTextil/OrdenCompraTextil_Insert';
