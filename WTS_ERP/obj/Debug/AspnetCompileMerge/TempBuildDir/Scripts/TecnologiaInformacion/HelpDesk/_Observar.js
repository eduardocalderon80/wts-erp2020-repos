var app_Ticket_Observar = (
    function (d, idpadre) {
        ...
        function load() {
            let par = _('txtpar_ticket_observar').value;
            if (!_isEmpty(par)) {
            ...
            _('btn_guardar_observar').addEventListener('click', req_guardar);
        }
        ...
        function req_guardar() {
            let requerido = required_item_class({ id: 'pnl_ticket_observar', clase: '_enty' });
            ...
            else { swal({ title: "Alert", text: 'Debe ingresar un motivo', type: "warning" }); return; }
        }

        function fn_guardar() {
            let urlaccion = 'TecnologiaInformacion/HelpDesk/Ticket_Update_Estado_Motivo';
            let oticket = _getParameter({ id: 'pnl_ticket_observar', clase: '_enty' });
            form = new FormData();
...
)(document, 'pnl_ticket_observar');

(function ini() {
    app_Ticket_Observar.load();
})();