    ...
    function (d, idpadre) {
        ...
        function fn_agregar() {
            ...
            if (req) {
                let html = `<tr>
                            <td>
                                <button type="button" class="btn btn-xs btn-danger" onclick="appPerfilUsuarioIndex.fn_remove(this)">
        ...
        function fn_load_personal() {
            ...
            if (lst_personal[0].brevete != '') { _('_Brevete').value = lst_personal[0].brevete; }
            if (lst_personal[0].pasaporte != '') { _('_TienePasaporte').value = lst_personal[0].pasaporte; $('#_TienePasaporte').change(); }
            ...
            /* Estudios */
            if (lst_personal[0].estudiosperu != '') { _('_EstudiosPeru').value = lst_personal[0].estudiosperu; }
        ...
        const _removeAll = (keys, arr) => { for (prop of keys) arr.delete(prop); return arr };
...
)(document, 'panelEncabezado_PerfilUsuarioIndex');
(
    function ini() {
        appPerfilUsuarioIndex.load();
        appPerfilUsuarioIndex.req_ini();
    }
)();
    ...
    _Get('RecursosHumanos/PerfilUsuario/GetAllData_PerfilUsuario?par=' + JSON.stringify(parametro))
        .then((resultado) => {
            ...
            if (rpta !== null) {
                ...
                ovariables.lst_area = rpta[0].area !== '' ? JSON.parse(rpta[0].area)[0] : [];
                ovariables.lst_subarea = rpta[0].subarea !== '' ? JSON.parse(rpta[0].subarea)[0] : [];
                ovariables.lst_equipo = rpta[0].equipo !== '' ? JSON.parse(rpta[0].equipo)[0] : [];
                ovariables.lst_cargo = rpta[0].cargo !== '' ? JSON.parse(rpta[0].cargo)[0] : [];
                ovariables.lst_region = rpta[0].region !== '' ? JSON.parse(rpta[0].region)[0] : [];
                ovariables.lst_provincia = rpta[0].provincia !== '' ? JSON.parse(rpta[0].provincia)[0] : [];
                ovariables.lst_distrito = rpta[0].distrito !== '' ? JSON.parse(rpta[0].distrito)[0] : [];
                ovariables.lst_institucioneducativa = rpta[0].institucioneducativa !== '' ? JSON.parse(rpta[0].institucioneducativa)[0] : [];
                ovariables.lst_regimeninstitucion = rpta[0].regimeninstitucion !== '' ? JSON.parse(rpta[0].regimeninstitucion)[0] : [];
                ovariables.lst_afonp = rpta[0].afponp !== '' ? JSON.parse(rpta[0].afponp)[0] : [];
                ovariables.lst_estadocivil = rpta[0].estadocivil !== '' ? JSON.parse(rpta[0].estadocivil)[0] : [];
                ovariables.lst_tipovivienda = rpta[0].tipovivienda !== '' ? JSON.parse(rpta[0].tipovivienda)[0] : [];
                ovariables.lst_gruposanguineo = rpta[0].gruposanguineo !== '' ? JSON.parse(rpta[0].gruposanguineo)[0] : [];
                ovariables.lst_licenciaconducir = rpta[0].licenciaconducir !== '' ? JSON.parse(rpta[0].licenciaconducir)[0] : [];



                //* Guarda ovariables
                //ovariables.lstperfil = rpta[0].lstperfil !== '' ? JSON.parse(rpta[0].lstperfil)[0] : [];
                //ovariables.lstregion = rpta[0].lstregion !== '' ? JSON.parse(rpta[0].lstregion) : [];
                //ovariables.lstprovincia = rpta[0].lstprovincia !== '' ? JSON.parse(rpta[0].lstprovincia) : [];
                //ovariables.lstdistrito = rpta[0].lstdistrito !== '' ? JSON.parse(rpta[0].lstdistrito) : [];
                ...
                //* Cambia valores combo
                const combobox = ['Region', 'Provincia', 'Distrito'];
                combobox.forEach(x => {
                    _(`_${x}`).value = appPerfilUsuarioIndex.ovariables.lstperfil[`Id${x}`];
                    $(`#_${x}`).change()
                });

                //* Crear familiares
                const arrfamiliares = ovariables.lstperfil.Familiares !== '' ? JSON.parse(ovariables.lstperfil.Familiares) : [];
                const objfamiliares = arrfamiliares.map(x => {
                    return `<tr>
                                        <td>
                                            <button type="button" class="btn btn-xs btn-danger" onclick="appPerfilUsuarioIndex.fn_remove(this)">
                ...
                //* Crear contactos
                let count = 0;
                const objcontactos = ovariables.lstperfil.Contactos !== '' ? JSON.parse(ovariables.lstperfil.Contactos) : [];
                objcontactos.forEach(x => {
                    count++;
                    _(`_Contacto${count}`).value = x.contacto;
                    _(`_Parentesco${count}`).value = x.parentesco;
                ...
                //* Llena inputs
                const objperfil = ovariables.lstperfil;
                const objremove = new Set(['IdRegion', 'IdProvincia', 'IdDistrito', 'Familiares', 'Contactos', 'IdPersonal', 'IdDetallePersonal', 'FotoPerfil']);
                const perfilkeys = [..._removeAll(objremove, new Set(Object.keys(objperfil)))];
                perfilkeys.forEach(x => {
                    _(`_${x}`).value = objperfil[x];
                });
...
/*
//function fn_load_ubigeo() {
//    const cbo_region = ovariables.lst_region.map(x => {
//        return `<option value="${x.idregion}">${x.region}</option>`
//    }).join('');
//    _('_Region').innerHTML = `<option value=''>Seleccione Departamento</option>${cbo_region}`;

//    _('_Region').onchange = function () {
//        const cbo_provincia = ovariables.lst_provincia.filter(x => x.idregion === this.value).map(x => {
//            return `<option value='${x.idprovincia}'>${x.provincia}</option>`
//        }).join('');
//        _('_Provincia').innerHTML = `<option value=''>Seleccione Provincia</option>${cbo_provincia}`;
//        _('_Distrito').innerHTML = `<option value=''>Seleccione Distrito</option>`;

//        // Para rules
//        $("#_Region, #_Provincia, #_Distrito").closest(".form-group").removeClass('has-error');
//    }
//    _('_Provincia').onchange = function () {
//        const cbo_distrito = ovariables.lst_distrito.filter(x => x.idprovincia === this.value).map(x => {
//            return `<option value="${x.iddistrito}">${x.distrito}</option>`
//        }).join('');
//        _('_Distrito').innerHTML = `<option value=''>Seleccione Distrito</option>${cbo_distrito}`;
//        // Para rules
//        $("#_Region, #_Provincia, #_Distrito").closest(".form-group").removeClass('has-error');
//    }
//    _('_Distrito').onchange = function () {
//        // Para rules
//        $("#_Region, #_Provincia, #_Distrito").closest(".form-group").removeClass('has-error');
//    }
//}
...
//    const cboRegion = ovariables.lstregion.map(x => {
//        return `<option value="${x.IdRegion}">${x.Region}</option>`
//    }).join('');
//    _('_Region').innerHTML = `<option value="">--</option>${cboRegion}`;

//    _('_Region').onchange = function () {
//        const cboProvincia = ovariables.lstprovincia.filter(x => x.IdRegion === this.value).map(x => {
//            return `<option value="${x.IdProvincia}">${x.Provincia}</option>`
//        }).join('');
//        _('_Provincia').innerHTML = `<option value="">--</option>${cboProvincia}`;
//        _('_Distrito').innerHTML = `<option value="">--</option>`;

//        // Para rules
//        $("#_Region, #_Provincia, #_Distrito").closest(".form-group").removeClass('has-error');
//    }
//    _('_Provincia').onchange = function () {
//        const cboDistrito = ovariables.lstdistrito.filter(x => x.IdProvincia === this.value).map(x => {
//            return `<option value="${x.IdDistrito}">${x.Distrito}</option>`
//        }).join('');
//        _('_Distrito').innerHTML = `<option value="">--</option>${cboDistrito}`;
