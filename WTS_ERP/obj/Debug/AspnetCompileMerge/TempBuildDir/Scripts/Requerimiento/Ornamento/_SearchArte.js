...
var app_searchArte =
    (
        function (d, idpadre) {
            var ovariables = {
                ...
                aRows: [],
                aRowsSelected: [],
                idTable: 'tbl_searchArte',
                isCheckBox: true, // [true: CheckBox, false: Radio]
                numPagina: 5,
                idTxtParametro: 'txtpar_searchArte',
                urlGet: 'Requerimiento/Ornamento/GetArteSearchAll_csv',
            ...
            function DataTableTemplate(arr) {
                const html = arr.map(x => {
                    return `<tr data-par="idArte:${x.IdRequerimientoDetalle},idrequerimiento:${x.IdRequerimientoDetalle}">
            ...
            function load() {
                ...
                // Events
                _('btnsearchArteAdd').addEventListener('click', fn_copiar_Arte, false);
                // Parameter
                cargaParameter();
            }

            function fn_copiar_Arte() {
                let arr_chk_select = Array.from(_('tbody_tbl_searchArte').getElementsByClassName('cls_chk_select')).filter(x => x.classList.value.indexOf('checked') >= 0);
                if (arr_chk_select.length > 0) {
                    ...
                    $("#modal_SearchArte").modal('hide');                    
                    appStages.fn_newart(-idrequerimientoDetalle,0);
                  
                } else {
                    _swal({ mensaje: 'Seleccione el Arte porfavor...!', estado: 'error' });
                }
            }
    ...
    )(document, 'panelEncabezado_searchArte');

(
    function ini() {
        app_searchArte.load();
        app_searchArte.req_ini();
    }
)();
