...
var Po = null;
var PoCliente = null;
var PoClienteEstilo = null;
var PoClienteEstiloDestino = null;
var PoClienteEstiloDestinoTallaColor = null;
var PoClienteProducto = null;
var PoClienteProductoDestino = null;
...
function LoadCustomer(data) {
    var html = "<option>Select</option>" + _comboFromJSON(JSON.parse(data), "IdCliente", "NombreCliente");
    FillSelect("cboCliente", html);
}
function LoadSupplier(data) {
    var html = "<option>Select</option>" + _comboFromJSON(JSON.parse(data), "IdProveedor", "NombreProveedor");
...
function LoadData(data) {
    if (data != "") {
        ...
        Po = JSON.parse(Data[0].Po);
        PoCliente = JSON.parse(Data[0].PoCliente);
        if (Po != null) {
            if (Po[0].IdTipo == "1") {
                PoClienteEstilo = JSON.parse(Data[0].PoClienteEstilo);
                PoClienteEstiloDestino = JSON.parse(Data[0].PoClienteEstiloDestino);
                PoClienteEstiloDestinoTallaColor = JSON.parse(Data[0].PoClienteEstiloDestinoTallaColor);
                LoadGarment();
            } else {
                PoClienteProducto = JSON.parse(Data[0].PoClienteProducto);
                PoClienteProductoDestino = JSON.parse(Data[0].PoClienteProductoDestino);
                LoadOther();
            }
        }
    } else {
...
function LoadOther() {
    var htmlDestino = "";
    var Destino = GroupBy(PoClienteProductoDestino, "IdClienteDireccion,Direccion,NombrePais,NombreCiudad");
    var nDestino = Destino.length;
    if (nDestino > 0) {
        htmlDestino += "<table class='table table-bordered'>";
        htmlDestino += "<thead><tr>";
        htmlDestino += "<tr>";
        htmlDestino += "<th class='text-center'>Select</th>";
        htmlDestino += "<th class='text-center'>Direccion</th>";
        htmlDestino += "</tr>";
        htmlDestino += "</thead>";
        htmlDestino += "<tbody>";
        for (var i = 0; i < nDestino; i++) {
            htmlDestino += "<tr>";
            htmlDestino += "<td class='text-center'><input type='radio' name='radioDireccion' onchange='LoadOtherDetail(" + Destino[i].IdClienteDireccion + ")' value='" + Destino[i].IdClienteDireccion + "'/></td>";
            htmlDestino += "<td class='text-center'>" + Destino[i].NombrePais + " - " + Destino[i].NombreCiudad + " - " + Destino[i].Direccion + "</td>";
            htmlDestino += "</tr>";
        }
        htmlDestino += "</tbody>";
        htmlDestino += "</table>";
        ...
        $("#divDetalle").empty();
        $("#divDestinoH").show();
        $("#divDestinos").append(htmlDestino);
    }
}
function LoadGarment() {

    var htmlDestino = "";
    var Destino = GroupBy(PoClienteEstiloDestino, "IdClienteDireccion,Direccion,NombrePais,NombreCiudad");
    var nDestino = Destino.length;
    if (nDestino > 0) {
        htmlDestino += "<table class='table table-bordered'>";
        htmlDestino += "<thead><tr>";
        ...
        for (var i = 0; i < nDestino; i++) {
            htmlDestino += "<tr>";
            htmlDestino += "<td class='text-center'><input type='radio' name='radioDireccion' onchange='LoadGarmentDetail(" + Destino[i].IdClienteDireccion + ")' value='" + Destino[i].IdClienteDireccion + "'/></td>";
            htmlDestino += "<td class='text-center'>" + Destino[i].NombrePais + " - " + Destino[i].NombreCiudad + " - " + Destino[i].Direccion + "</td>";
            htmlDestino += "</tr>";
        }
        htmlDestino += "</tbody>";
...
function LoadGarmentDetail(idClienteDireccion) {
    if (PoClienteEstilo != null && PoClienteEstiloDestino != null && PoClienteEstiloDestinoTallaColor != null) {
        nPoClienteEstilo = PoClienteEstilo.length;
        nPoClienteEstiloDestino = PoClienteEstiloDestino.length;
        nPoClienteEstiloDestinoTallaColor = PoClienteEstiloDestinoTallaColor.length;

        var Data = null;
        if (idClienteDireccion > 0) {
            Data = PoClienteEstiloDestinoTallaColor.filter(function (e) { return (e.IdClienteDireccion === idClienteDireccion) });
        } else {
            Data = PoClienteEstiloDestinoTallaColor;
        }
            ...
            for (var i = 0; i < nPoClienteEstiloColor; i++) {
                ...
                for (var x = 0; x < nClienteTalla; x++) {
                    ...
                    if (obj != null) {
                        ...
                        htmlDetail += "<th class='text-center'><input type='text' class='form-control text-center pkqty' data-params='" + params + "' value='' placeholder='" + parseInt(obj.Cantidad) + "' onkeypress='return DigitimosDecimales(event,this);' /></th>";
                    } else {
...
function LoadOtherDetail(idClienteDireccion) {
    var nPoClienteProductoDestino = PoClienteProductoDestino.length;
    if (nPoClienteProductoDestino > 0) {
        var Data = null;
        if (idClienteDireccion > 0) {
            Data = PoClienteProductoDestino.filter(function (e) { return (e.IdClienteDireccion === idClienteDireccion) });
        } else {
            Data = PoClienteProductoDestino;
        }
        var nData = Data.length;
        if (nData > 0) {
            var params = "";
            ...
            for (var i = 0; i < nData; i++) {
                ...
                htmlDetail += "<td class='text-center'><input type='text' class='form-control text-center pkqty' value='0' data-params='" + params + "' onkeypress='return DigitimosDecimales(event,this);' /></td>";
                htmlDetail += "</tr>";
            }
            htmlDetail += "</tbody>";
            htmlDetail += "</table>";
...
function AddDestination() {
    ...
    if (Po != null) {
        ...
        Get('PackingList/PackingList/GetClientAddress?par=' + IdCliente, LoadDestination);
...
function LoadOtherDestination(obj) {
    
    if (obj.checked) {
        if (Data != "") {
            ClienteDireccion = JSON.parse(Data);
            var nClienteDireccion = ClienteDireccion.length;
            var html = "<option value=''>Select</option>" + _comboFromJSON(ClienteDireccion, "IdClienteDireccion", "Direccion");
            $("#cboClienteDireccion").append(html);
            $("#divDestinos").hide();
            $("#divDestino").show();
            $("#hfOtherDestination").val(1);
        } else {
            alert('Client Address not found.');
        }
    } else {
        $("#divDestinos").show();
        $("#divDestino").hide();
        $("#hfOtherDestination").val(0);
    }
    ...
    if (Po != null) {
        if (Po[0].IdTipo == "1") {
            ...
            if (nPackingListPoClienteEstiloTallaColor > 0) {
                ...
                if ($("#hfOtherDestination").val() == "1") {
                    IdClienteDireccion = $("#cboClienteDireccion").val();
                } else {
            ...
            if (nPackingListPoClienteProducto > 0) {
                ...
                if (nPackingListPoCliente > 0) {
                    ...
                    if ($("#hfOtherDestination").val() == "1") {
                        ...
                        IdClienteDireccion = PackingListPoClienteProducto[0].IdClienteDireccion;
                    }
