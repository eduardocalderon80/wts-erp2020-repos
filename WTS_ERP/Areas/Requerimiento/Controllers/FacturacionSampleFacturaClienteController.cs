...
{
    public class FacturacionSampleFacturaClienteController : Controller
    {
        private readonly IFacturaClienteService _facturaClienteServicio;
        public FacturacionSampleFacturaClienteController(IFacturaClienteService facturaClienteService)
        {
            this._facturaClienteServicio = facturaClienteService;
        }

        // GET: Requerimiento/FacturacionSampleFacturaCliente
        public ActionResult _FacturaClienteFacturacionSample()
        {
            return View();
        }

        public string GetFacturaClienteLoadNew_JSON()
        {
            string par = _.Get("par");
            par = _.addParameter(par, "IdGrupoPersonal", _.GetUsuario().IdGrupoComercial.ToString());
            return _facturaClienteServicio.GetFacturaClienteLoadNew_JSON(par);
        }
        ...
        public string GetRequerimientoMuestraDetalle_JSON()
        {
            ...
            return _facturaClienteServicio.GetRequerimientoMuestraDetalle_JSON(listaRequerimientosSeleccionados);
        }

        public string SaveNewFacturaCliente_JSON()
        {
            string sParFacturaClienteModel = _.Post("FacturaClienteJSON");
            string sParFacturaClienteDetalle = _.Post("FacturaClienteDetalleJSON");
            string sParListaColoresModificados = _.Post("ListaColoresModificadosJSON");

            FacturaClienteViewModels facturaCliente = JsonConvert.DeserializeObject<FacturaClienteViewModels>(sParFacturaClienteModel);
            List<FacturaClienteDetalleViewModels> listaFacturaClienteDetalle = JsonConvert.DeserializeObject<List<FacturaClienteDetalleViewModels>>(sParFacturaClienteDetalle);
            List<FacturaClienteDetalleViewModels> listaColoresModificados = JsonConvert.DeserializeObject<List<FacturaClienteDetalleViewModels>>(sParListaColoresModificados);

            facturaCliente.Usuario = _.GetUsuario().Usuario;
            facturaCliente.Ip = "";
            facturaCliente.HostName = "";

            GenerarArchivo(Request.Files["File_Packinglist"], facturaCliente, "File_Packinglist");
            GenerarArchivo(Request.Files["File_GuiaAerea"], facturaCliente, "File_GuiaAerea");
            GenerarArchivo(Request.Files["File_DeclaracionJurada"], facturaCliente, "File_DeclaracionJurada");
            GenerarArchivo(Request.Files["File_CertificadoOrigen"], facturaCliente, "File_CertificadoOrigen");

            int idFacturaCliente = _facturaClienteServicio.SaveNewFacturaCliente_JSON(facturaCliente
                , listaFacturaClienteDetalle, listaColoresModificados);

            string mensaje = _.Mensaje("new", idFacturaCliente > 0, null, idFacturaCliente);
            return mensaje;
        }

        public string SaveEditFacturaCliente_JSON()
        {
            string sParFacturaClienteModel = _.Post("FacturaClienteJSON");
            string sParFacturaClienteDetalle = _.Post("FacturaClienteDetalleJSON");
            string sParListaColoresModificados = _.Post("ListaColoresModificadosJSON");
            string sParListaOrdenPedidos = _.Post("ListaOrdenesPedidosJSON");
            ...
            int idFacturaCliente = _facturaClienteServicio.SaveEditFacturaCliente_JSON(facturaCliente
                , listaFacturaClienteDetalle, listaColoresModificados, sParListaOrdenPedidos);

            string mensaje = _.Mensaje("edit", idFacturaCliente > 0, null, idFacturaCliente);
            return mensaje;
        }

        private void GenerarArchivo(HttpPostedFileBase file, FacturaClienteViewModels facturaCliente, string NombrePost_File)
        {
            if (file != null)
            {
                Random aleatorio = new Random();

                //var file = Request.Files["File_Packinglist"];
                ...
                switch (NombrePost_File)
                {
                    case "File_Packinglist":
                        facturaCliente.NombreArchivoPackingListGenerado = nombrearchivogenerado;
                        break;
                    case "File_GuiaAerea":
                        facturaCliente.NombreArchivoGuiaAereaGenerado = nombrearchivogenerado;
                        break;
                    case "File_DeclaracionJurada":
                        facturaCliente.NombreArchivoDeclaracionJuradaGenerado = nombrearchivogenerado;
                        break;
                    case "File_CertificadoOrigen":
                        facturaCliente.NombreArchivoCerfiticadoOrigenGenerado = nombrearchivogenerado;
                        break;
                }
                ...
                //string rutaFile = @rutaFileServer + "erp/Requerimiento/FacturacionSample/FacturaCliente"
                string rutaFile = Server.MapPath("~/Content/FacturacionSampleFacturaCliente/") + nombrearchivogenerado;
                System.IO.File.WriteAllBytes(rutaFile, bfile);
            }
            
        }

        public string SaveCancelarFacturaCliente_JSON()
        {
            int idfacturacliente = Convert.ToInt32(_.Post("IdFacturaCliente"));
            string usuario = _.GetUsuario().Usuario;
            int rows = _facturaClienteServicio.SaveCancelarFacturaCliente_JSON(idfacturacliente, usuario);
            string mensaje = _.Mensaje("edit", rows > 0, null, rows);
            return mensaje;
        }

        public string GetFacturaClienteLoadEdit_JSON()
        {
            string sPar = _.Get("par");
            sPar = _.addParameter(sPar, "IdGrupoPersonal", _.GetUsuario().IdGrupoComercial);
            string data = _facturaClienteServicio.GetFacturaClienteLoadEdit_JSON(sPar);
            return data;
        }

        public FileResult DownlaodFileAdjuntoDataAdcional()
        {
            string sParNombreOriginal = _.Get("NombreArchivoOriginal");
            string sParNombreGenerado = _.Get("NombreArchivoGenerado");
            string rutaFile = Server.MapPath("~/Content/FacturacionSampleFacturaCliente/") + sParNombreGenerado;
            byte[] arrByte = System.IO.File.ReadAllBytes(rutaFile);
            return File(arrByte, System.Net.Mime.MediaTypeNames.Application.Octet, sParNombreOriginal);
        }

        /// <summary>
        /// METODOS PARA GENERAR LA FACTURA CLIENTE - PO
        /// </summary>
        /// <returns></returns>
        public FileResult GeneratePO_PDF()
        {
            int idFacturaCliente = Convert.ToInt32(_.Get("IdFacturaCliente"));
            string data = _facturaClienteServicio.GetFacturaClienteForReporteByIdJSON(idFacturaCliente);
            FacturaClienteViewModels po = JsonConvert.DeserializeObject<FacturaClienteViewModels>(data);
            List<FacturaClienteDetalleViewModels> ListaFacturaClienteDetalle = JsonConvert.DeserializeObject<List<FacturaClienteDetalleViewModels>>(po.FacturaClienteDetalle);
            ...
            string numeroCodigoPoCliente = Convert.ToDateTime(po.FechaCreacion).Year.ToString() + "-" + Convert.ToDateTime(po.FechaCreacion).Month.ToString("00") + "-" + po.NumeroFacturaCliente.Substring(po.NumeroFacturaCliente.IndexOf("-") + 1, 5);
            ...
            try
            {
                ...
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    //// LOGO
                    tablaCaceberaLogo.AddCell(DataBloque1_CabeceraLogo_LogoEmpresa(jo_empresa));
                    //// NUMERO POCLIENTE
                    tablaCaceberaLogo.AddCell(DataBloque1_CabeceraLogo_NumeroFactura(po, MediumFontBold));
                    //// FECHAS
                    tablaCaceberaLogo.AddCell(DataBloque1_CabeceraLogo_Fechas(po, SmallFontBoldWhite, baseColorGencianPerlado, SmallFont));
                    ...
                    tablaPrincipal.AddCell(DataBloque2_InformacionCliente(jo_datosCliente, po.NumeroFacturaCliente, SmallFontBoldWhite, baseColorGencianPerlado, SmallFont));
                    ...
                    ExportToPdf_DetallePOClienteEstilo(document, ListaRequerimientoMuestra, ListaFacturaClienteDetalle,
        ...
        private PdfPCell DataBloque1_CabeceraLogo_LogoEmpresa(JObject jo_empresa)
        {
            ...
            return celdaImagenLogo;
        }

        private PdfPCell DataBloque1_CabeceraLogo_NumeroFactura(FacturaClienteViewModels oFacturaCliente,
            Font MediumFontBold)
        {
            // -- NUMERO POCLIENTE
            string numeroCodigoPoCliente = Convert.ToDateTime(oFacturaCliente.FechaCreacion).Year.ToString() + "-" + Convert.ToDateTime(oFacturaCliente.FechaCreacion).Month.ToString("00") + "-" + oFacturaCliente.NumeroFacturaCliente.Substring(oFacturaCliente.NumeroFacturaCliente.IndexOf("-") + 1, 5);
            ...
            return celdaNumeroPoCliente;
        }

        private PdfPCell DataBloque1_CabeceraLogo_Fechas(FacturaClienteViewModels oFacturaCliente,
            Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont)
        {
            ...
            PdfPCell celdaSubTablaCabeceraLogo2 = new PdfPCell(new Paragraph(Convert.ToDateTime(oFacturaCliente.FechaCreacion).ToString("MM-dd-yyyy"), SmallFont));
            ...
            PdfPCell celdaSubTablaCabeceraLogo4 = new PdfPCell(new Paragraph(Convert.ToDateTime(oFacturaCliente.FechaActualizacion).ToString("MM-dd-yyyy"), SmallFont));
            ...
            PdfPCell celdaSubTablaCabeceraLogo6 = new PdfPCell(new Paragraph(Convert.ToDateTime(oFacturaCliente.FechaActualizacion).ToString("MM-dd-yyyy"), SmallFont));
        ...
        public void ExportToPdf_DetallePOClienteEstilo(Document document,
            ...
            List<FacturaClienteDetalleViewModels> listaFacturaClienteDetalle,
            JObject jo_datosCliente, FacturaClienteViewModels oFacturaCliente,
            Font MediumFontBold, Font SmallFont, Font SmallFontBold, Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFontWhite)
        {
            ...
            {
                List<FacturaClienteDetalleViewModels> listaReqMuestraByIdReq = listaFacturaClienteDetalle.FindAll(x => x.IdRequerimiento == itemRequerimiento.IdRequerimiento);
                ...
                document.Add(DataBloque3Detalle_Comentarios(itemRequerimiento, jo_datosCliente, oFacturaCliente, SmallFontBoldWhite, baseColorGencianPerlado, SmallFont));
                document.Add(new Paragraph(" "));  // SPACIO EN BLANCO
            }
        }

        private PdfPTable DataBloque3Detalle_Comentarios(RequerimientoMuestraViewModels oRequerimientoMuestra,
            JObject jo_datosClienteJSON, FacturaClienteViewModels oFacturaCliente,
            Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont)
        {
            ...
            PdfPCell valorComentario3 = new PdfPCell(new Paragraph(oFacturaCliente.Comentario, SmallFont));
            valorComentario2.FixedHeight = 20f;
        ...
        private PdfPTable DataBloque2Detalle_OrdersDetails_CounterSample(List<FacturaClienteDetalleViewModels> listaFacturaClienteDetalle,
            Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont, Font SmallFontBold)
        {
            List<FacturaClienteDetalleViewModels> listaIdClienteTalla = (from data in listaFacturaClienteDetalle
                                                                         select new
                                                                         {
                                                                             IdClienteTalla = data.IdClienteTalla,
                                                                             NombreClienteTalla = data.NombreClienteTalla
                                                                         }).Distinct()
                                       .Select(x => new FacturaClienteDetalleViewModels { IdClienteTalla = x.IdClienteTalla, NombreClienteTalla = x.NombreClienteTalla })
                                       .ToList();
            ...
            List<FacturaClienteDetalleViewModels> listaIdClienteColor = (from data in listaFacturaClienteDetalle
                                                                         select new
                                                                         {
                                                                      ...
                                                                      .Select(x => new FacturaClienteDetalleViewModels { IdClienteColor = x.IdClienteColor, NombreClienteColor = x.NombreClienteColor, EsContramuestraFacturableCliente = x.EsContramuestraFacturableCliente, PrecioContraMuestra = x.PrecioContraMuestra })
                                                                      .ToList();
            ...
            Contenido_OrdersDetails_CounterSample(listaIdClienteColor, listaIdClienteTalla, listaFacturaClienteDetalle, tablaPrincipalOrderDetails, totalColumnasDinamicas, SmallFont, SmallFontBold);

            return tablaPrincipalOrderDetails;
        }

        private PdfPTable DataBloque2Detalle_OrdersDetails_Sample(List<FacturaClienteDetalleViewModels> listaFacturaClienteDetalle,
            Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont, Font SmallFontBold)
        {
            List<FacturaClienteDetalleViewModels> listaIdClienteTalla = (from data in listaFacturaClienteDetalle
                                                                         select new
            ...
            List<FacturaClienteDetalleViewModels> listaIdClienteColor = (from data in listaFacturaClienteDetalle
                                       ...
                                       .Select(x => new FacturaClienteDetalleViewModels { IdClienteColor = x.IdClienteColor, NombreClienteColor = x.NombreClienteColor, PrecioMuestra = x.PrecioMuestra })
                                       .ToList();
            ...
            SetearAnchoColumnasDinamicas(tablaPrincipalOrderDetails, totalColumnasDinamicas, totalColumnasFijas);
            Cabecera_OrdersDetails_Sample(listaIdClienteTalla, tablaPrincipalOrderDetails, SmallFontBoldWhite, baseColorGencianPerlado);
            Contenido_OrdersDetails_Sample(listaIdClienteColor, listaIdClienteTalla, listaFacturaClienteDetalle, tablaPrincipalOrderDetails, totalColumnasDinamicas, SmallFont, SmallFontBold);
        ...
        private void Contenido_OrdersDetails_Sample(List<FacturaClienteDetalleViewModels> listaColores,
            List<FacturaClienteDetalleViewModels> listaTallas, List<FacturaClienteDetalleViewModels> listaReqMuestra,
        ...
        private void Contenido_OrdersDetails_CounterSample(List<FacturaClienteDetalleViewModels> listaColores,
            List<FacturaClienteDetalleViewModels> listaTallas, List<FacturaClienteDetalleViewModels> listaReqMuestra,
            PdfPTable tablaPrincipalOrderDetails, int totalColumnasDinamicas,
            Font SmallFont, Font SmallFontBold)
        {
            ...
            foreach (var itemClienteColor in listaColores)
            {
                ...
                foreach (var itemColumnasClienteTalla in listaTallas)
                {
                    FacturaClienteDetalleViewModels dataTalla = new FacturaClienteDetalleViewModels();
        ...
        private void Cabecera_OrdersDetails_Sample(List<FacturaClienteDetalleViewModels> listaTallas, PdfPTable tablaPrincipalOrderDetails, Font SmallFontBoldWhite,
        ...
        private PdfPCell DataBloque2_InformacionCliente(JObject jo_clienteJSON, string numeroFacturaCliente, Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont)
        {
            PdfPTable tableContent = new PdfPTable(1);
            tableContent.WidthPercentage = 40;
            tableContent.HorizontalAlignment = Element.ALIGN_RIGHT;
            ...
            //// VALOR
            PdfPCell cellDato4 = new PdfPCell(new Phrase(numeroFacturaCliente, SmallFont));
        ...
        //// PARA EL BLOQUE 3
        private PdfPCell DataBloque3_TotalPoCantidad(FacturaClienteViewModels po, Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont)
        {
            PdfPTable tableContent = new PdfPTable(1);
            tableContent.WidthPercentage = 40;
            tableContent.HorizontalAlignment = Element.ALIGN_RIGHT;
        ...
        private PdfPCell DataBloque3_TotalPoCosto(FacturaClienteViewModels po, Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont)
        {
            PdfPTable tableContent = new PdfPTable(1);
            tableContent.WidthPercentage = 40;
            tableContent.HorizontalAlignment = Element.ALIGN_RIGHT;
        ...
        private PdfPCell DataBloque3_TotalPoTerms(FacturaClienteViewModels po, Font SmallFontBoldWhite, BaseColor baseColorGencianPerlado, Font SmallFont)
        {
            PdfPTable tableContent = new PdfPTable(1);
            tableContent.WidthPercentage = 40;
            tableContent.HorizontalAlignment = Element.ALIGN_RIGHT;
