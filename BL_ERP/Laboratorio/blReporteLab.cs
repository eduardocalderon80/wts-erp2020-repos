...
{
    public class blReporteLab : blLog
    {
        public List<ReporteLab> Reporte_Export_Pruebas(string nombreBD, string fabrica, string cliente, string fechadesde, string fechahasta, string statusfinal, string tipoprueba, string reportetecnico, string numeropartida)
        {
            List<ReporteLab> listpruebas = new List<ReporteLab>();
            ...
            using (SqlConnection con = new SqlConnection(conexion))
            {
                ...
                {
                    con.Open();
                    daReporteLab daReporteLab = new daReporteLab();
                    listpruebas = daReporteLab.Reporte_Export_Pruebas(con, fabrica, cliente, fechadesde, fechahasta, statusfinal, tipoprueba, reportetecnico, numeropartida);
                }
                catch (Exception ex)
                {
                    listpruebas = null;
        ...
        public List<ReporteLab> Reporte_Export_LeadTime(string nombreBD, string fabrica, string cliente, string fechadesde, string fechahasta, string statusfinal, string tipoprueba, string reportetecnico, string numeropartida)
        {
            List<ReporteLab> listpruebas = new List<ReporteLab>();
            string conexion = nombreBD ?? Util.Default;
            ...
            using (SqlConnection con = new SqlConnection(conexion))
            {
                try
                {
                    con.Open();
                    daReporteLab daReporteLab = new daReporteLab();
                    listpruebas = daReporteLab.Reporte_Export_LeadTime(con, fabrica, cliente, fechadesde, fechahasta, statusfinal, tipoprueba, reportetecnico, numeropartida);
                }
                catch (Exception ex)
                {
                    listpruebas = null;
        ...
        public List<ReporteLab> Reporte_Export_TestSummay(string nombreBD, string fabrica, string cliente, string fechadesde, string fechahasta, string statusfinal, string tipoprueba, string reportetecnico, string numeropartida)
        {
            List<ReporteLab> listpruebas = new List<ReporteLab>();
            string conexion = nombreBD ?? Util.Default;
            ...
            using (SqlConnection con = new SqlConnection(conexion))
            {
                try
                {
                    con.Open();
                    daReporteLab daReporteLab = new daReporteLab();
                    listpruebas = daReporteLab.Reporte_Export_TestSummay(con, fabrica, cliente, fechadesde, fechahasta, statusfinal, tipoprueba, reportetecnico, numeropartida);
                }
                catch (Exception ex)
                {
                    listpruebas = null;
        ...
        public List<ReporteLab> Laboratorio_Reporte_Export_LeadTimeKPI(string nombreBD, int mes, int anio)
        {
            List<ReporteLab> listpruebas = new List<ReporteLab>();
            string conexion = nombreBD ?? Util.Default;
            ...
            using (SqlConnection con = new SqlConnection(conexion))
            {
                try
                {
                    con.Open();
                    daReporteLab daReporteLab = new daReporteLab();
                    listpruebas = daReporteLab.Laboratorio_Reporte_Export_LeadTimeKPI(con, mes, anio);
                }
                catch (Exception ex)
                {
                    listpruebas = null;
        ...
        public byte[] crearExcel_Reporte_Pruebas(string titulo, string strListaCabeceraTabla, string strDatosFilasTabla, string strTituloDocumento, int tiporeporte)
        {
            byte[] result = null;
            try
            {
                using (ExcelPackage package = new ExcelPackage())
                {
                    ...
                    for (int i = 1; i < arrayFilasTabla.Length; i++)
                    {
                        ...
                        for (int j = 0; j < columnas.Length; j++)
                        {
                            ...
                            {
                                if (j == 8) { worksheet.Cells[indexFila, indexColumna].Value = Convert.ToInt32(columnas[j]); }                                
                                else
                                { worksheet.Cells[indexFila, indexColumna].Value = columnas[j].Trim(); }
                            }
                            else { worksheet.Cells[indexFila, indexColumna].Value = columnas[j].Trim(); }
